...
int selfId = 2;
float selfChance = 0;
int totalNumberOfLevels = 3;
float X = 73.05;
float Y = 40.74;
int numPack = 0;
float  tp = 100;
float selfLeveNodesData[2][4];
int level;
...
void sendSelfData(int level, int id, float voltage, float x, float y, float sC)
{
  CCPACKET packet1;
    ...
    packet1.data[5] = getDecimal(x);
    packet1.data[6] = int(y); 
    packet1.data[7] = getDecimal(y);
    packet1.data[8] = int(sC); 
    packet1.data[9] = getDecimal(sC);
...
//        {  
      ...
      {
         if (rxPacket->data[1] == 1) // ** TO BE CHANGED **
         {
          saveData();
          node1 = true; // ** TO BE CHANGED **
         }
          
        //nPack ++;
        
        if (node1 == true) // ** TO BE CHANGED **
        {
        ...
        if ((rxPacket->data[0] == selfLevel) && (rxPacket->data[1] != 0))
           {
            ...
            if (nbPacketsFromSameLevel == 1) //** TO BE CHANGED **//
            {
